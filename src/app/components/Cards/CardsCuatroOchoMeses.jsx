...
import actividades from '../../../mocks/cuatroOchoMeses.json'
...
export default function CardsCuatroOchoMeses (){
    ...
    const cuatroOchoMeses = actividades.actividades.filter((actividad) => actividad.edad === "4 a 8 meses" && actividad.tipo === "principal");
    const cuatroOchoMesesExtra = actividades.actividades.filter((actividad) => actividad.edad === "4 a 8 meses" && actividad.tipo === "extra");
    return (
                            ...
                            <motion.div className=" flex justify-start items-center gap-5 mb-5 mt-10 sm:gap-5 h-[220px] cursor-grab" drag='x' dragConstraints={{right: 480.5, left: -480.5}}>
                                ...
                                {cuatroOchoMeses.map((actividad) => (
                                    <div key={actividad.id} className=" w-[300px] h-[180px] flex flex-col items-center justify-between bg-white bg-opacity-70 shadow-lg backdrop-filter backdrop-blur-md border border-opacity-18 p-5 rounded-md  sm-m:min-w-[300px] sm:w-[300px] cursor-grab ">
                                        ...
                                        <Link href={'https://agesandstages.com/wp-content/uploads/2023/05/6-month_ASQ-3-Parent-Activities_Spanish.pdf'} target='_blank' className=" flex flex-col gap-2">
