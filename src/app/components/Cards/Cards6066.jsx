...
import actividades from '../../../mocks/6066Meses.json'
...
export default function Cards6066 (){
    ...
    const sesentaSesentaiseisMeses = actividades.actividades.filter((actividad) => actividad.edad === "60 a 66 meses" && actividad.tipo === "principal");
    const sesentaSesentaiseisMesesExtra = actividades.actividades.filter((actividad) => actividad.edad === "60 a 66 meses" && actividad.tipo === "extra");
    return (
                ...
                <motion.div className='w-screen overflow-hidden flex flex-col items-center justify-center'>
                    {
                        ...
                        {sesentaSesentaiseisMeses.map((actividad) => (
                            <div key={actividad.id} className=" w-[300px] h-[180px] flex flex-col items-center justify-between bg-white bg-opacity-70 shadow-lg backdrop-filter backdrop-blur-md border border-opacity-18 p-5 rounded-md  sm-m:min-w-[300px] sm:w-[300px] cursor-grab">
                                ...
                                <Link href={'https://agesandstages.com/wp-content/uploads/2023/05/5-year_ASQ-3-Parent-Activities_Spanish.pdf'} target='_blank' className=" flex flex-col gap-2">
                    ...
                    
                    {
                        mostrarCards && (
                            ...
                            <motion.div className="  overflow-hidden flex justify-start items-center gap-5 mb-10 mt-10 sm:gap-5 h-[375px] cursor-grab px-10" drag='x' dragConstraints={{right: 3040, left: -3040}} initial={{ x: 3040 }}>
                                ...
                                {sesentaSesentaiseisMesesExtra.map((actividad) => (
